...
TEST_CASE(test_decode_splitkv_combine)
{
    ...
    std::vector<TestConfig> configs = {
        ...
        // Asymmetric K/O — the main cases the algorithmic generation targets
        {1, 256, 64, 128, 4, 4, 2},
        {1, 512, 64, 256, 4, 4, 4},
        {1, 256, 128, 64, 4, 4, 2},
        {1, 512, 128, 256, 4, 4, 4},
        ...
        {1, 256, 128, 192, 4, 4, 2},
        {1, 512, 256, 128, 4, 4, 4},
        // Asymmetric K/O with GQA
        {1, 256, 64, 128, 8, 2, 2},
        {1, 512, 128, 256, 8, 4, 4},
        // Asymmetric K/O with M > 1
        {8, 256, 64, 128, 4, 4, 2},
        {16, 512, 64, 256, 4, 4, 4},
    };
...
TEST_CASE(benchmark_fmha_decode)
{
    constexpr auto test_body = [](const splitkv::Problem& splitkv_prob) {
        ...
        std::cout << "Problem: batch=" << splitkv_prob.batch << ", nhead=" << splitkv_prob.nhead
                  ...
                  << ", N=" << splitkv_prob.N << ", K=" << splitkv_prob.K
                  << ", O=" << splitkv_prob.O << ", num_splits=" << splitkv_prob.num_splits
                  << std::endl;
        ...
        for(std::size_t sol_idx = 0; sol_idx < splitkv_solutions.size(); ++sol_idx)
        {
            ...
            try
            {
                ...
                {
                    splitkv_kernel.launch(nullptr, splitkv_grid, splitkv_block)(
                        q_device.data(),
                        k_device.data(),
                        v_device.data(),
                        o_acc_device.data(),
    ...
    std::vector<size_t> ms{1, /*16*/};
    std::vector<size_t> ns{256, /*1024*/};
    std::vector<size_t> ks{/*32, 64, 128, 192,*/ 256};
    std::vector<size_t> os{/*32, 64, 96, 128, 192,*/ 256};
    for(auto m : ms)
    {
        for(auto n : ns)
        {
